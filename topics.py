# Combined Knowledge & Culture Categories
# LIVE topics only. The default state for a topic is *quarantined* (see quarantine/quarantine.json);
# a topic appears here only after its lesson has been reviewed and meets planning/lesson-standard.md.
# Re-add topics batch by batch as they are improved.
KNOWLEDGE_AND_CULTURE_CATEGORIES = {
    "Cross-Domain Foundational Activities": [
        "Bug hunting and magnifying glass observation",
        "Learning to tie knots and bows",
        "Making a picture diary or journal",
        "Sink or float experiments",
        "Making bead and pasta necklace patterns",
        "Sorting a treasure box of buttons, shells, and stones",
        "Show and tell presentations",
        "Creating a nature display from a walk",
        "Painting a group mural together",
        "Building a blanket fort"
    ],
    "Cross-Domain Foundational Concepts": [
        "Mirror neurons and learning by watching",
        "Pretend play and symbolic thinking"
    ],
    "Human Experience - Social": [
        "Attachment and the biology of caregiving"
    ],
    "Creative and Expressive - Performing Arts": [
        "Rhythm and entrainment"
    ],
    "Creative and Expressive - Visual Arts": [
        "Drawing, sketching, and scribbling",
        "Painting with various materials",
        "Sculpture and 3D art with clay/playdough",
        "Photography and picture-taking",
        "Printmaking and stamping",
        "Pottery and clay work",
        "Art installation and display"
    ],
    "Physical World - Life Sciences": [
        "Seeds and germination",
        "Decomposition and compost"
    ],
    "Systems and Activities - Mathematical": [
        "Counting, cardinality, and infinity"
    ],
    "Learning and Growth - Exploration": [
        "Sensory exploration activities"
    ],
    "Human Experience - Linguistic": [
        "Phonemes and the sounds of language"
    ],
    "Human Experience - Cultural": [
        "How conversation differs across cultures"
    ]
}

PHYSICAL_ACTIVITIES = {
    "Physical and Movement - Individual Sports": [
        "Tennis and racquet sports",
        "Golf and putting games",
        "Track and field (running, jumping, throwing)",
        "Swimming and water activities",
        "Gymnastics and tumbling",
        "Martial arts and self-defense",
        "Cycling and bike riding",
        "Skateboarding and balance sports",
        "Rock climbing and bouldering",
        "Archery and target sports",
        "Bowling and precision rolling",
        "Table tennis and paddle sports",
        "Badminton and shuttlecock games",
        "Figure skating and ice activities",
        "Surfing and wave sports",
        "Skiing and snow sports",
        "Wrestling and grappling",
        "Boxing and striking sports"
    ],
    "Physical and Movement - Team Sports": [
        "Soccer and football",
        "Basketball and hoop games",
        "Baseball and softball",
        "American football and tackle games",
        "Volleyball and net sports",
        "Hockey (ice and field)",
        "Lacrosse and stick sports",
        "Rugby and contact sports",
        "Cricket and bat sports",
        "Water polo and pool games",
        "Ultimate frisbee and disc sports",
        "Handball and court games",
        "Dodgeball and throwing games",
        "Capture the flag and territory games",
        "Relay races and team challenges"
    ],
    "Physical and Movement - Individual Physical": [
        "Running, walking, and locomotion",
        "Jumping, hopping, and movement games",
        "Climbing and balancing activities",
        "Stretching and flexibility exercises",
        "Strength activities and resistance play",
        "Endurance and cardiovascular games",
        "Fine motor skill development",
        "Coordination and agility training",
        "Dance and rhythmic movement",
        "Yoga and mindful movement",
        "Calisthenics and bodyweight exercises"
    ],
    "Physical and Movement - Outdoor Adventure": [
        "Hiking and nature walks",
        "Camping and outdoor living play",
        "Orienteering and navigation games",
        "Geocaching and treasure hunting",
        "Nature observation and wildlife tracking",
        "Outdoor obstacle courses",
        "Survival skills and wilderness play",
        "Fishing and angling activities",
        "Hunting simulation games",
        "Mountain biking and trail riding",
        "Kayaking and paddling sports",
        "Sailing and wind sports"
    ],
    "Physical and Movement - Aquatic Sports": [
        "Swimming techniques and strokes",
        "Diving and underwater activities",
        "Water safety and rescue practice",
        "Synchronized swimming and water dance",
        "Water skiing and towing sports",
        "Snorkeling and underwater exploration",
        "Competitive swimming and racing",
        "Water aerobics and pool exercises",
        "Beach volleyball and sand sports",
        "Surfing and wave riding",
        "Paddleboarding and balance on water"
    ],
    "Physical and Movement - Winter Sports": [
        "Skiing (downhill and cross-country)",
        "Snowboarding and board sports",
        "Ice skating and rink activities",
        "Ice hockey and puck sports",
        "Sledding and snow sliding",
        "Snowshoeing and winter hiking",
        "Figure skating and ice dance",
        "Curling and precision ice sports",
        "Bobsledding and speed sports",
        "Snow fort building and winter construction"
    ],
    "Physical and Movement - Combat Sports": [
        "Karate and striking arts",
        "Judo and throwing arts",
        "Taekwondo and kicking arts",
        "Brazilian Jiu-Jitsu and ground fighting",
        "Wrestling and takedowns",
        "Fencing and sword sports",
        "Mixed martial arts basics",
        "Self-defense and protection skills",
        "Aikido and redirecting force"
    ],
    "Physical and Movement - Mind-Body Activities": [
        "Yoga poses and sequences",
        "Tai Chi and flowing movements",
        "Qigong and energy exercises",
        "Pilates and core strengthening",
        "Meditation and breathing exercises",
        "Balance and stability training",
        "Body awareness and proprioception",
        "Relaxation and tension release",
        "Mindful walking and movement",
        "Progressive muscle relaxation"
    ],
    "Physical and Movement - Performance Arts": [
        "Dance choreography and routines",
        "Cheerleading and acrobatic movements",
        "Rhythmic gymnastics with props",
        "Color guard and flag work",
        "Drum line and marching band",
        "Synchronized movement activities",
        "Performance stunts and tricks",
        "Stage movement and blocking",
        "Movement storytelling",
        "Musical theater and integrated performance"
    ],
    "Physical and Movement - Motor Skills": [
        "Fine motor skill development",
        "Hand-eye coordination games",
        "Gross motor skill practice",
        "Bilateral coordination exercises",
        "Sensory integration activities",
        "Visual-motor skill development",
        "Reaction time and quick response games",
        "Sequential movement patterns",
        "Cross-lateral movement exercises",
        "Spatial awareness and body mapping"
    ]
}
